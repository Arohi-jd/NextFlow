...
__name(isGroqModel, "isGroqModel");
function isGeminiRecoverableError(error) {
  const message = error instanceof Error ? error.message.toLowerCase() : String(error).toLowerCase();
  return message.includes("429") || message.includes("quota") || message.includes("rate limit") || message.includes("404") && message.includes("not found");
}
__name(isGeminiRecoverableError, "isGeminiRecoverableError");
var VIDEO_EXTENSION_RE = /\.(mp4|mov|webm|avi|mkv|m4v|ogv|3gp)(\?|#|$)/i;
...
__name(resolveImageUrls, "resolveImageUrls");
...
var llmTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      const output = isGroqModel(resolvedPayload.model) ? await callGroq(resolvedPayload) : await callGeminiWithFallback(resolvedPayload);
      await prisma.nodeExecution.update({
        where: { id: payload.nodeExecutionId },
        data: {
          status: "success",
